...
export async function onRequest(context) {
  ...
  try {
    ...
    return jsonResponse(await runMarketScan(windowDays));
  } catch (error) {
    return jsonResponse({ error: error.message || 'Research scan failed' }, 500);
  }
}
...
function parseRssItems(xml, fallbackUrl) {
  ...
  return blocks.map(block => {
    ...
    return { title, source, url, summary: rawSummary.slice(0, 260), publishedAt };
...
function isTelcoRelevantArticle(article, region) {
  const normalized = normalizeComparableText(`${article.title || ''} ${article.summary || ''} ${article.source || ''}`);
...
function extractDirectSourceArticles(page, html, region) {
  ...
  while ((match = linkPattern.exec(html))) {
    ...
    const article = { title, summary, source: page.source, url, publishedAt: new Date().toISOString(), score: scoreStory(title, summary, new Date()) + 6 };
